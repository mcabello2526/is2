...
*/
public class GUIVincularPeliculaAGenero extends JFrame implements IGUI {
	...
	public GUIVincularPeliculaAGenero() {
		setTitle("Vincular película a género");
	...
	private JButton getBtnVincular() {
		if(btnVincular == null) {
			btnVincular = new JButton("Vincular");
			btnVincular.setForeground(textoNegro);
			btnVincular.setBackground(btnVerde);
			btnVincular.setBounds(210, 230, 110, 35);
			btnVincular.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if(validar()) {
						...
						Controlador.getInstance().accion(Evento.VINCULAR_PELICULA_A_GENERO, datosVincular);
					}
				}
			});
		}
	...
	private boolean validar() {
		try {
			int idPelicula = Integer.parseInt(txtIdPelicula.getText().trim());
			int idGenero = Integer.parseInt(txtIdGenero.getText().trim());
			
			if(idPelicula < 0 || idGenero < 0) {
				JOptionPane.showMessageDialog(this,  "Los IDs deben ser números enteros positivos.", "ERROR", JOptionPane.ERROR_MESSAGE);
				return false;
			}
			return true;
	...
	public void actualizar(int evento, Object datos) {
		switch(evento){
		case Evento.RES_VINCULAR_PELICULA_A_GENERO_OK:
			JOptionPane.showMessageDialog(this,  "Vinculación realizada correctamente.");
			dispose();
			break;
		case Evento.RES_VINCULAR_PELICULA_A_GENERO_KO:
			JOptionPane.showMessageDialog(this, "Error al vincular, asegurese de que ambos IDs existen", "ERROR", JOptionPane.ERROR_MESSAGE);
			break;
		}
	}
}