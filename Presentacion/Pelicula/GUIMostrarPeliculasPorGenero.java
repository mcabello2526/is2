...
*/
public class GUIMostrarPeliculasPorGenero extends JFrame implements IGUI {
	...
	public GUIMostrarPeliculasPorGenero() {
		setTitle("Mostrar películas por generos");
	...
	private JLabel getLblTitulo() {
		if(lblTitulo == null) {
			lblTitulo = new JLabel("Mostrar películas por géneros");
	...
	private JPanel getPanelBuscar() {
		if(panelBuscar == null) {
			...
			JLabel lblId = new JLabel("ID género:");
			lblId.setBounds(30, 25, 100, 20);
			panelBuscar.add(lblId);
			
			txtIdGenero = new JTextField();
			txtIdGenero.setBounds(140, 23, 120, 25);
			panelBuscar.add(txtIdGenero);
			...
			btnBuscar.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (validar()) {
						int id = Integer.parseInt(txtIdGenero.getText().trim());
						Controlador.getInstance().accion(Evento.GUI_MOSTRAR_PELICULAS_POR_GENERO,  id);
					}
				}
			});
	...
	private boolean validar() {
		String id = txtIdGenero.getText().trim();
		try {
	...
	public void actualizar(int evento, Object datos) {
		switch(evento) {
		case Evento.RES_MOSTRAR_PELICULAS_POR_GENERO_OK:
			...
			break;
		case Evento.RES_MOSTRAR_PELICULAS_POR_GENERO_KO:
			tableModel.setRowCount(0);
			JOptionPane.showMessageDialog(null,  "No se han encontrados películas para este género", "ERROR", JOptionPane.ERROR_MESSAGE);
			break;
		}
	}
}