...
public class GUIMostrarTodasLasNacionalidades extends JFrame implements IGUI {
	...
	public GUIMostrarTodasLasNacionalidades() {
		setTitle("Mostrar todas las nacionalidades");
		...
		Controlador.getInstance().accion(Evento.MOSTRAR_TODAS_LAS_NACIONALIDADES, null);
	...
	private JLabel getLblTitulo() {
		if(lblTitulo == null) {
			lblTitulo = new JLabel("Mostrar todas las nacionalidades");
	...
	private JPanel getPanelTabla() {
		if(panelTabla == null) {
			...
			tableModel = new DefaultTableModel(new Object[] {"ID", "Nombre", "Continente"}, 0);
	...
	public void actualizar(int evento, Object datos) {
		switch(evento) {
		case Evento.RES_MOSTRAR_TODAS_LAS_NACIONALIDADES_OK:
			List<TNacionalidad> lista = (List<TNacionalidad>) datos;
			tableModel.setRowCount(0);
			
			for(TNacionalidad p : lista) {
				Object[] rowData = {
				p.getId(), p.getNombre(),
				p.getContinente()
			};
			...
			break;
		case Evento.RES_MOSTRAR_TODAS_LAS_NACIONALIDADES_KO:
			tableModel.setRowCount(0);
			JOptionPane.showMessageDialog(null,  "No hay nacionalidades registradas", "AVISO", JOptionPane.WARNING_MESSAGE);
			break;
		}
	}
}